...
function initModeButtons() {
  ...
  clientModeBtn.addEventListener("click", () => {
    const disable = ["connecting", "connected"].includes(
      window.connection_status.get(),
    );
    if (disable) return;
...
function lockFormsWhenConnected() {
  window.connection_status.subscribe((status) => {
    ...
    const clientModeBtn = document.getElementById("client-mode");
    const serverModeBtn = document.getElementById("server-mode");
    clientModeBtn.disabled = disable;
    serverModeBtn.disabled = disable;

    const clientForm = document.querySelector("#client-options form");
    const clientFormFields = clientForm.querySelectorAll(
    ...
    const serverForm = document.querySelector("#server-options form");
    const serverFormFields = serverModeBtn.querySelectorAll(
      "input, select, textarea",
    );
    ...
    const clientConnectBtn = document.getElementById("client-connect-btn");
    clientConnectBtn.innerHTML = disable ? "Disconnect" : "Connect";
    const serverConnectBtn = document.getElementById("server-connect-btn");
    serverConnectBtn.innerHTML = disable ? "Stop server" : "Start server";
  });
}
